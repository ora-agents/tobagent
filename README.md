...
# Or with pip
aegra dev --host 0.0.0.0 --port 2025
```

Open LangGraph Studio: <https://smith.langchain.com/studio/?baseUrl=http://127.0.0.1:2025>

For local Docker PostgreSQL, do not set `DATABASE_URL` to a remote database.
Use the local `POSTGRES_*` values from `.env.example`, which match the
`docker-compose.yml` generated by `aegra dev`.
