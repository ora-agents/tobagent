...
A multi-agent AI assistant platform with a **LangGraph + Python backend** and a **Next.js frontend**.
...
2. **FastAPI Sidecar** — REST API (mounted via `langgraph.json` `http.app` at `src/api/server.py:app`). Handles CRUD for agent profiles, knowledge bases, skills, MCP servers, user auth, and document uploads.
...
- `DynamicConfigMiddleware` (`src/middleware/dynamic_config_middleware.py`) — the key middleware. Intercepts model calls to inject per-request system prompts, filter tools, create dynamic subagent tools (`call_agent_*`), inject MCP tools, and switch models at runtime. Reads agent profiles from the database.
- `ModelRetryMiddleware` / `ToolRetryMiddleware` — retry logic for model and tool failures

**Tools** (`src/tools/`):
- `rag_tool.py` — LanceDB-backed RAG search. Each agent/KB gets its own LanceDB table. Uses async-native LanceDB API + OpenAI embeddings.
- `websearch_tool.py` — Web search (Tavily)
- `fetch_tool.py` — URL fetching
- `skill_tool.py` — `read_skill` tool to load custom skill content from DB
- `mcp_tools.py` — Dynamic MCP (Model Context Protocol) tool integration via `src/utils/mcp.py` pool manager
- `link_check_tools.py`, `pricing_tools.py` — docs-agent-specific tools

**Database**: SQLAlchemy ORM in `src/utils/db.py`. Uses PostgreSQL (via `DATABASE_URL`) in production, falls back to local SQLite (`chat_langchain.db`) for development. Tables: `agent_profiles`, `knowledge_bases`, `skills`, `mcp_servers`, `users`, `client_profiles`. Schema migrations are done via `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` in the FastAPI lifespan handler.

### Frontend (Next.js)

Next.js 16 + React 19 in `frontend/`. Uses Tailwind CSS + Radix UI (shadcn/ui pattern in `components/ui/`).
...
`src/api/auth.py` implements LangGraph's auth framework. Optional `LANGGRAPH_AUTH_SECRET` env var gates access via `X-Auth-Key` header. User identity comes from `Authorization: Bearer <user_id>`. LangGraph Studio users are detected automatically and bypass restrictions.
...
- `LANGSMITH_API_KEY` / `LANGSMITH_PROJECT` — Tracing via LangSmith.
