...
class BackendGeneratedIdUpsertInput(BaseModel):
    """Base schema for resources whose new IDs are generated by the backend."""

    model_config = ConfigDict(extra="forbid")


class UpsertAgentProfileInput(BackendGeneratedIdUpsertInput):
    """Input for creating or updating an agent profile."""

    existing_agent_id: str = Field(
        default="",
        description="Existing agent ID to update. Leave empty when creating; new IDs are always generated by the backend.",
    )
    name: str
    description: str = ""
    system_prompt: str = ""
...
class UpsertAgentProfileTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        db = SessionLocal()
        try:
            agent_id = str(kwargs.get("existing_agent_id") or "").strip()
            profile = None
            if agent_id:
...
class UpsertSkillInput(BackendGeneratedIdUpsertInput):
    """Input for creating or updating a skill."""

    existing_skill_id: str = Field(
        default="",
        description="Existing skill ID to update. Leave empty when creating; new IDs are always generated by the backend.",
    )
    name: str = Field(
        default="",
...
class UpsertSkillTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        db = SessionLocal()
        try:
            skill_id = str(kwargs.get("existing_skill_id") or "").strip()
            skill = (
                db.query(SkillTable).filter(
                    SkillTable.id == skill_id,
...
class UpsertFormInput(BackendGeneratedIdUpsertInput):
    """Input for creating or updating a form definition."""

    existing_form_id: str = Field(
        default="",
        description="Existing form ID to update. Leave empty when creating; new IDs are always generated by the backend.",
    )
    name: str
    description: str = ""
    category: str = ""
...
class UpsertFormTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        db = SessionLocal()
        try:
            form_id = str(kwargs.get("existing_form_id") or "").strip()
...
class UpsertMcpServerInput(BackendGeneratedIdUpsertInput):
    """Input for creating or updating an MCP server."""

    existing_mcp_id: str = Field(
        default="",
        description="Existing MCP server ID to update. Leave empty when creating; new IDs are always generated by the backend.",
    )
    name: str
    url: str
...
class UpsertMcpServerTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        db = SessionLocal()
        try:
            mcp_id = str(kwargs.get("existing_mcp_id") or "").strip()
...
class UpsertKnowledgeBaseInput(BackendGeneratedIdUpsertInput):
    """Input for creating or updating knowledge-base metadata."""

    existing_knowledge_base_id: str = Field(
        default="",
        description="Existing knowledge base ID to update. Leave empty when creating; new IDs are always generated by the backend.",
    )
    name: str
    description: str = ""
...
class UpsertKnowledgeBaseTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        db = SessionLocal()
        try:
            kb_id = str(kwargs.get("existing_knowledge_base_id") or "").strip()
