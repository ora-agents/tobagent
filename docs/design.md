...
- **WSIRI Blue / Primary** (`{colors.primary}` - #164199): The logo blue. Used for primary buttons, active nav, links, selected states, and key icon accents. Do not use it as an input or textarea focus border.
...
- **Focus Halo** (`{colors.focus-halo}` - rgba(22, 65, 153, 0.16)): Optional accessible focus state for buttons, menus, tabs, and other command controls. Text inputs and textareas do not use a halo in the default product UI.
...
- Focus-visible states need a clear visual indicator on command controls such as buttons, menus, tabs, switches, and icon actions. Text inputs and textareas are the exception: they keep the same fill, avoid borders/rings, and rely on the text caret plus the user's typed content for editing state.
...
- **Chat input:** Soft filled panel with stable focus treatment. Avoid visible borders, focus rings, or focus-time fill changes on the textarea itself.
...
- Primary actions, command-control focus rings, selected navigation, and links still use WSIRI Blue, but not as bare text on black. Use filled primary controls with white text, a verified lighter link color, or another state treatment that meets WCAG contrast.
- In dark mode, use charcoal fill changes instead of blue-gray outlines. Blue appears on focused or selected command controls through halo/fill, not borders. Inputs and textareas keep their default fill when focused.

### Inputs & Forms

- Default inputs and textareas use `{colors.surface-soft}` or `{colors.surface-raised}` fill and `{colors.ink}` text with no border.
- Focused inputs and textareas keep the same fill, text color, radius, shadow, and supporting icon color as the default state. Do not add a visible border, outline, halo, blue ring, or brighter background on focus. This also applies to their wrapper panels and chat input containers: avoid `focus-within` rings, shadows, and fill changes.
...
- Use blue for active state, command-control focus state, and primary action consistently. Do not use blue for input or textarea focus chrome.
...
- Don't use borders as the default way to draw buttons, inputs, cards, or list items. Use color-block fills, spacing, typography, and command-control focus rings first; inputs and textareas should not gain focus borders or rings.
