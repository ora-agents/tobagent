...
- **Data views:** Favor predictable rows, dividers, tabs, and toolbars over decorative cards.
...
- Default inputs use `{colors.surface-panel}`, `{colors.hairline}`, and `{colors.ink}`.
- Focus state uses `{colors.primary}` border and a 3-4px `{colors.primary}` ring at 12-16% opacity.
- Placeholder and helper text use `{colors.muted}`.
- Errors use `{colors.error}` border/text and a very light red fill.
